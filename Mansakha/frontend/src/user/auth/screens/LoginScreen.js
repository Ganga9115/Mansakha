...
import { useUserLogin } from '../../shared/services/hooks';
...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await login({ token: data.token, accountType: 'user' });
    } catch (err) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      await apiClient.post('/api/auth/user/change-password', { newPassword }, tempToken);
      await login({ token: tempToken, accountType: 'user' });
    } catch (err) {
