// Demo dataset for Build Prompt Section 10: "realistic sample users,
...
// and dummy user (DUMMY-0001) seedDummyAccounts.js already created, and
// never touches or duplicates them.
//
// Every check-in is driven through the REAL pipeline - the same
// analyzeInteraction() (real Gemini call) + insert logic routes/user.js's
...
// stable identifier (jurisdiction name+level, user docket_number, existing
// interaction count), matching seedDummyAccounts.js's pattern. Re-running
// resumes any user that didn't finish its check-in history last time
...
const { analyzeInteraction } = require('../../ai/ai');
...
// check-ins for the same user don't repeat verbatim.
...
async function callAnalyzeWithRetry(userId, text, label) {
  const backoffs = [4000, 8000, 16000];
  let lastErr;
  for (let attempt = 0; attempt <= backoffs.length; attempt++) {
    try {
      const analysis = await analyzeInteraction(userId, text);
...
async function ensureDemoUser(config) {
  const { data: existing, error: findErr } = await supabase.from('users').select('user_id').eq('docket_number', config.docketNumber).maybeSingle();
  if (findErr) throw new Error(`Could not check existing user ${config.docketNumber}: ${findErr.message}`);
  if (existing) {
    console.log(`Using existing demo user: ${config.docketNumber} (${config.fullName})`);
  ...
  const { data: user, error: userErr } = await supabase
    ...
    .select('user_id')
    .single();
  if (userErr) throw new Error(`Could not create demo user ${config.docketNumber}: ${userErr.message}`);

  const { error: identityErr } = await supabase.from('user_identity').insert({
  ...
  const { error: enrollErr } = await supabase.from('users').update({ enrolled_at: dateFromDaysAgo(earliestDaysAgo).toISOString() }).eq('user_id', user.user_id);
  if (enrollErr) throw new Error(`Could not backdate enrolled_at for ${config.docketNumber}: ${enrollErr.message}`);

  console.log(`Created demo user: ${config.docketNumber} (${config.fullName})`);
  return user.user_id;
}

async function countExistingInteractions(userId) {
  const { count, error } = await supabase.from('interactions').select('interaction_id', { count: 'exact', head: true }).eq('user_id', userId);
  if (error) throw new Error(`Could not count interactions for user ${userId}: ${error.message}`);
  return count || 0;
}

// ===== The real check-in pipeline, replicated from routes/user.js POST /checkin =====

async function doRealCheckin(userId, channelId, text, label, maps) {
  const { data: interaction, error: interactionErr } = await supabase
    .from('interactions')
    .insert({ user_id: userId, channel_id: channelId, transcript_ref: text })
    .select('interaction_id')
    .single();
  if (interactionErr) throw new Error(`Could not record interaction for ${label}: ${interactionErr.message}`);

  const analysis = await callAnalyzeWithRetry(userId, text, label);
  ...
  if (analysis.riskLevel === 'High' || analysis.riskLevel === 'Critical') {
    const { data: alert, error: alertErr } = await supabase
      .from('alerts')
      .insert({ user_id: userId, distress_score_id: scoreRow.score_id, alert_status_id: maps.openAlertStatusId })
      .select('alert_id')
      .single();
    if (alertErr) throw new Error(`Could not create alert for ${label}: ${alertErr.message}`);
    alertId = alert.alert_id;

    const { data: userRow, error: userRowErr } = await supabase.from('users').select('jurisdiction_id').eq('user_id', userId).single();
    if (userRowErr) throw new Error(`Could not look up jurisdiction for alert routing (${label}): ${userRowErr.message}`);

    const { data: districtOfficials, error: officialsErr } = await supabase
      .from('official_roles')
      .select('official_id, roles(role_name)')
      .eq('jurisdiction_id', userRow.jurisdiction_id)
      .is('revoked_at', null);
...
async function checkEscalating(userId) {
  const { data: scores, error } = await supabase.from('distress_scores').select('score_value, computed_at').eq('user_id', userId).order('computed_at', { ascending: false }).limit(3);
  if (error) throw new Error(`Could not fetch scores to check escalation for user ${userId}: ${error.message}`);
  if (!scores || scores.length < 3) return false;
  return isEscalatingTrend([...scores].reverse().map((s) => s.score_value));
}

async function runCheckinsForUser(config, userId, maps) {
  const label = `${config.docketNumber} (${config.fullName})`;
  const existingCount = await countExistingInteractions(userId);
  ...
  if (!config.isEscalating) {
    ...
    for (const [idx, ci] of remaining.entries()) {
      ...
      try {
        const result = await doRealCheckin(userId, config.channelId, ci.text, `${label} checkin#${num}`, maps);
        ...
        break; // stop this user's remaining check-ins; safely resumable next run
      }
  ...
  // Escalating-trend user: base 3 first, then fallback (more severe) check-ins
  ...
  let fallbackIdx = 0;
  let confirmed = await checkEscalating(userId);
  while (!confirmed && fallbackIdx < ESCALATING_FALLBACK.length) {
    ...
    try {
      const result = await doRealCheckin(userId, config.channelId, text, `${label} fallback-checkin#${fallbackIdx + 1}`, maps);
    ...
    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) return;
    fallbackIdx += 1;
    confirmed = await checkEscalating(userId);
  }
}

// ===== User configuration =====

function buildUserConfigs(maps, districts) {
  ...
  return [
    ...
    { docketNumber: 'DEMO-0002', fullName: 'Ramesh Naik', email: 'ramesh.naik@example.com', caseTypeId: CT['Murder'], jurisdictionId: districts.bengaluru, caseStage: 'Trial', channelId: CH['Web Portal'], languageId: LANG.en, checkins: withDaysAgo([LOW[1], LOW[2]], [40, 15]) },
    ...
    { docketNumber: 'DEMO-0004', fullName: 'Arjun Solanki', email: 'arjun.solanki@example.com', caseTypeId: CT['Rape'], jurisdictionId: districts.lucknow, caseStage: 'Compensation', channelId: CH.SMS, languageId: LANG.hi, checkins: withDaysAgo([LOW[0], LOW[3]], [50, 20]) },
    ...
    { docketNumber: 'DEMO-0007', fullName: 'Geeta Shinde', email: 'geeta.shinde@example.com', caseTypeId: CT['Gang Rape'], jurisdictionId: districts.bengaluru, caseStage: 'Rehabilitation', channelId: CH['Web Portal'], languageId: LANG.te, checkins: withDaysAgo([MODERATE[2], MODERATE[0]], [25, 7]) },
    { docketNumber: 'DEMO-0008', fullName: 'Vikram Chauhan', email: 'vikram.chauhan@example.com', caseTypeId: CT['Grievous Hurt'], jurisdictionId: districts.mysuru, caseStage: 'Compensation', channelId: CH['Mobile App'], languageId: LANG.ta, checkins: withDaysAgo([MODERATE[3], MODERATE[1], MODERATE[2]], [33, 11, 4]) },
    ...
    { docketNumber: 'DEMO-0011', fullName: 'Anita Jadhav', email: 'anita.jadhav@example.com', caseTypeId: CT['Rape'], jurisdictionId: districts.pune, caseStage: 'Rehabilitation', channelId: CH['Web Portal'], languageId: LANG.mr, checkins: withDaysAgo([HIGH[2], HIGH[3]], [18, 2]) },
    { docketNumber: 'DEMO-0012', fullName: 'Deepak Rathod', email: 'deepak.rathod@example.com', caseTypeId: CT['Arson'], jurisdictionId: districts.bengaluru, caseStage: 'Compensation', channelId: CH['Mobile App'], languageId: LANG.bn, checkins: withDaysAgo([HIGH[0], HIGH[1], HIGH[3]], [22, 9, 1]) },
...
async function finalReport(userConfigs, maps) {
  console.log('\n=== Demo dataset verification ===');
  const counts = { Low: 0, Moderate: 0, High: 0, Critical: 0, None: 0 };
  const userIdByDocket = {};

  for (const config of userConfigs) {
    const { data: user, error } = await supabase.from('users').select('user_id').eq('docket_number', config.docketNumber).maybeSingle();
    if (error) throw new Error(`Could not verify user ${config.docketNumber}: ${error.message}`);
    if (!user) {
      console.warn(`  ${config.docketNumber}: user row missing (seed did not complete)`);
      continue;
    }
    userIdByDocket[config.docketNumber] = user.user_id;
  ...
  console.log('\nRisk-level distribution across demo users:', counts);

  const escConfig = userConfigs.find((c) => c.docketNumber === ESCALATING_DOCKET);
  const escUserId = userIdByDocket[ESCALATING_DOCKET];
  ...
  if (escUserId) {
    const { data: last3, error: last3Err } = await supabase
      ...
      .order('computed_at', { ascending: false })
      .limit(3);
    if (last3Err) throw new Error(`Could not fetch escalating user's scores: ${last3Err.message}`);
    ...
    console.warn(`WARNING: escalating-trend user ${ESCALATING_DOCKET} was not found.`);
  }

  const demoUserIds = Object.values(userIdByDocket);
  const { data: openAlerts, error: alertsErr } = await supabase
    .from('alerts')
    .select('alert_id, triggered_at, user_id, distress_scores(score_value, risk_levels(name))')
  ...
  const docketByUserId = Object.fromEntries(Object.entries(userIdByDocket).map(([d, v]) => [v, d]));
  const nameByDocket = Object.fromEntries(userConfigs.map((c) => [c.docketNumber, c.fullName]));
  console.log(`\nOpen alerts among demo users: ${(openAlerts || []).length}`);
  for (const a of openAlerts || []) {
    const docket = docketByUserId[a.user_id];
...
async function main() {
  ...
  const userConfigs = buildUserConfigs(maps, districts);

  console.log(`\nSeeding ${userConfigs.length} demo users with real check-in histories (this makes real Gemini API calls and can take a few minutes)...\n`);

  for (const config of userConfigs) {
    const userId = await ensureDemoUser(config);
    await runCheckinsForUser(config, userId, maps);
    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
      ...
      await finalReport(userConfigs, maps);
      process.exit(1);
    }
  }

  const report = await finalReport(userConfigs, maps);

  console.log('\nDemo dataset seed complete.');
  console.log(`Users: ${userConfigs.length}, distribution: ${JSON.stringify(report.counts)}, escalating trend confirmed: ${report.escalating}, open alerts: ${report.openAlertCount}.`);
}
