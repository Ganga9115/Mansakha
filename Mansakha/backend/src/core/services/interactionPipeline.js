...
// Shared by user/routes/user.routes.js's /checkin, /chat, and /sos - resolves
...
async function recordInteraction({ userId, channelName, transcriptText }) {
  ...
  const transcriptPath = `${userId}/${crypto.randomUUID()}.txt`;
  ...
  const { data: interaction, error: interactionError } = await supabase
    .from('interactions')
    .insert({ user_id: userId, channel_id: channelRow.channel_id, transcript_ref: transcriptPath, transcript_length: transcriptText.length })
