...
// state-wise comparison snapshot, per compute function per tier (this IS
...
// This is National Admin's own dedicated copy of what used to be the single
...
router.get(
  ...
  async (req, res) => {
    ...
    const [investigationProgress, threatProtection, compensationRelief, agencyReferralVolume, legalAidFunnel, agencyReferralVolumeByState, stateRiskComposition] = await Promise.all([
      ...
      // Average distress score + risk-tier composition per state - the same
      // view Ministry's own Analysis page used to show nationwide
      // (unrestricted); National Admin's own jurisdiction-scoped equivalent,
      // since Ministry's page has been retired in favour of this one.
      computeRiskCompositionByChild(jurisdictionId, true),
    ]);

    return ok(res, { trend, severityDistribution, interventionPhases, investigationProgress, threatProtection, compensationRelief, agencyReferralVolume, legalAidFunnel, agencyReferralVolumeByState, stateRiskComposition });
  }
);
...
// dead copy-paste from when the admin hierarchy was scaffolded. National
// Admin keeps GET /users (search-by-docket, above) and PATCH /users/:userId
...
router.patch(
  ...
  async (req, res) => {
    ...
    // migration_034: caseStage removed entirely - no staff role has any
    // case-stage authority any more.
...
router.get(
  ...
  async (req, res) => {
    ...
    // One grouped query for every role+jurisdiction's queue health, plus one
    // row-level query for the actual open referrals behind those counts -
    // both single round trips regardless of roster size, same "no query per
    // official" fix the counsellor-performance route above already applies.
    // The row-level query is what turns a bare "6 open" into an actual
    // worklist (docket number, case type, current risk level, days open)
    // instead of a dead end - the same "cap the box height, never the data"
    // philosophy the Staffing Gaps list below already follows.
    const [{ rows: queueRows }, { rows: openReferralRows }] = await Promise.all([
      pool.query(
    ...
    // Grouped by the SAME role+jurisdiction key queueByKey uses - an open
    // referral is scoped to a role within a jurisdiction, not to a specific
    // official (see this route's own header comment), so two officials who
    // genuinely share a scope correctly see the identical worklist rather
    // than an arbitrary split of it.
    ...
    for (const r of openReferralRows) {
      ...
      const entry = {
        ...
        daysOpen,
        // Same real 7-day threshold agencyEscalationChecker.js's own
        // automatic District Collector escalation fires on - "overdue" here
        // means exactly what the system already treats as overdue, not a
        // separately-invented UI-only number.
        overdue: daysOpen >= STALE_REFERRAL_DAYS,
      };
      if (!openReferralsByKey.has(key)) openReferralsByKey.set(key, []);
      openReferralsByKey.get(key).push(entry);
    }
...
router.get(
  ...
  async (req, res) => {
    ...
    const [roleGapResults, { rows: stationGaps }, { rows: totalDistrictRows }, { rows: totalStationRows }] = await Promise.all([
      Promise.all(
      ...
      // 8 districts than against 400, so both counts below turn the raw gap
      // list into an actual percentage instead of leaving the reader to
      // guess the size of the subtree it's drawn from.
      pool.query(`select count(*) as count from jurisdictions where jurisdiction_id = any($1::uuid[]) and level = 'district'`, [jurisdictionIds]),
      pool.query(`select count(*) as count from police_stations where jurisdiction_id = any($1::uuid[]) and deleted_at is null`, [jurisdictionIds]),
    ]);
...
// ===== Detailed PDF Reports (National tier) =====
// Replaces the old single-snapshot/single-target design: a National report
// carries a full state-wise comparison snapshot (reportSnapshot.js's
// computeStateWiseSnapshot) over a real calendar period (reportPeriods.js),
// rendered to a real PDF (reportPdf.js), and routed to Ministry via
// report_recipients instead of a single target_jurisdiction_id.

const VALID_PERIOD_TYPES = ['weekly', 'monthly', 'quarterly', 'custom'];

// National's report always goes straight to Ministry itself - nothing exists
// above National, so there is no optional-extra-recipient case at all here
// (unlike District/State). Any `recipients` body entries are rejected
// outright rather than silently ignored, since a client sending them almost
// certainly expects them to mean something.
function resolveNationalReportRecipients(requestedRecipients) {
  if (requestedRecipients && requestedRecipients.length > 0) {
    throw Object.assign(new Error('A National report has no optional recipients - it always goes to Ministry only'), { status: 400 });
  }
  return [{ recipientType: 'ministry', jurisdictionId: null, isPrimary: true }];
}
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!asDraft) {
      try {
        recipientsToInsert = resolveNationalReportRecipients(req.body.recipients);
      } catch (err) {
    ...
    try {
      computedSnapshot = await computeStateWiseSnapshot(jurisdictionId, periodStart, periodEnd, periodType);
    } catch (err) {
...
// this updates ONLY the caller's own recipient row (the single Ministry row,
// for a National report) - never the report's own sender.
router.patch(
...
// report_recipients instead of the old single target_jurisdiction_id. A
// National tier realistically only ever has an outbox (nothing routes a
// jurisdiction-type report_recipients row to a 'national' jurisdiction_id in
// this design), but the route stays symmetric with District/State rather
// than special-casing that away.
router.get(
  '/reports',
  verifyToken,
  requireRole(['Administration', 'Ministry']),
