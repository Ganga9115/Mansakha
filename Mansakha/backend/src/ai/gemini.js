...
user of an atrocity under India's SC/ST (Prevention of Atrocities) Act, as part of a
...
distress-monitoring app for users of an atrocity under India's SC/ST (Prevention of
Atrocities) Act. The user just sent you one chat message. Do two things at once:
...
exactly these fields:
{
  ...
  "suggestedIntervention": "<EXACTLY one of: Counselling, Medical, Witness Protection,
    ...
    never a decision.>",
  "reply": "<your supportive chat reply to the user, 2-4 short sentences, plain
...
async function generateProactiveContactMessage(recentTrendSummary) {
  const prompt = `You are "Mansakha", a supportive check-in companion for a user of an
...
user of an atrocity under India's SC/ST (Prevention of Atrocities) Act, based on
their latest check-in. Write a short, factual, professional summary (2-4 sentences)
of what the user reported - no diagnosis, no clinical labels, no legal conclusions,
...
Analyze this raw dump of user interactions, chat logs, and case notes.
...
3. SYNDICATE DETECTION: If you notice recurring mentions of similar modus operandi, specific locations, or similar threats across multiple DIFFERENT users, flag it under "emerging_risks" as a potential organized intimidation syndicate.
