...
// Investigating Officer - copied from dwo/routes/dwo.routes.js (the
// template); only ROLE_NAME differs. Logs FIR/chargesheet/bail status as
// notes against a referral (see agency_referral_notes' generic shape).
// Also the one role that SETS Accused Status - the real signal Threat Tier
// is computed from (see core/services/threatAssessment.js). This is IO's
// actual statutory function (custody/bail/absconding tracking), not a
// guess - Protection Officer reads the resulting tier read-only.
...
router.get('/referrals', async (req, res) => {
  ...
  return ok(res, {
    referrals: rows.map((r) => ({
      ...
      accusedStatus: r.metadata?.accusedStatus || null,
...
// Sets Accused Status - a real, structured field (replacing a free-text
// guess) that Threat Tier is computed from on every subsequent read. Not
// stored as a tier itself - the tier also depends on sos_events, which
// changes independently of this field, so it's always computed fresh.
