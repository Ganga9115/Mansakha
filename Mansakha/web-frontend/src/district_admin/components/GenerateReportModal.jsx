...
import { useGenerateReport, useJurisdictionOptions } from '../services/hooks';

// District Admin's own "Generate Report" modal - a case-wise report (every
// case in the district's jurisdiction listed individually, see
// ReportSnapshotView's `cases` table) submitted upward to District's own
// State (required, added server-side - never sent by this form) plus
// optional direct cc's to National/Ministry (the "Share With" checklist
// below - only the CHECKED optional rows go into the request's `recipients`
// array).
...
export default function GenerateReportModal({ jurisdictionId, onClose, onGenerated }) {
  ...
  const [includeMinistry, setIncludeMinistry] = useState(false);
  const [submitError, setSubmitError] = useState(null);
  const [submitting, setSubmitting] = useState(null); // null | 'draft' | 'submit'

  const generateReport = useGenerateReport();

  // Best-effort labels for the checklist's locked/informational rows - a
  // correct generic label beats a wrong specific one, so these fall back to
  // generic text below whenever the lookup hasn't resolved yet. Mirrors
  // ministry/components/ReportBuilder.jsx's own derivation: a district's
  // own state is its `parentId` on the level='district' lookup rows; the
  // national row is the sole level='national' entry.
  ...
  const myDistrictRow = districtOptions.data?.jurisdictions?.find((d) => d.jurisdictionId === jurisdictionId);
  const myStateName = stateOptions.data?.jurisdictions?.find((s) => s.jurisdictionId === myDistrictRow?.parentId)?.name;
  const nationalRoot = nationalOptions.data?.jurisdictions?.[0];
  ...
  const buildPayload = (asDraft) => {
    ...
    const recipients = [];
    if (includeNational && nationalRoot?.jurisdictionId) {
      recipients.push({ type: 'jurisdiction', jurisdictionId: nationalRoot.jurisdictionId });
    }
    if (includeMinistry) recipients.push({ type: 'ministry' });
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h3 className="font-bold text-sm text-gray-800">Generate Case-Wise Report</h3>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-gray-600">{myStateName || 'Your State Admin'} (State Admin)</span>
                </div>
              ...
              <div className="flex items-center justify-between bg-white border border-gray-200 rounded-lg px-3 py-2.5">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={includeNational} onChange={(e) => setIncludeNational(e.target.checked)} className="w-4 h-4 accent-brand-400" />
                  <span className="text-xs font-semibold text-gray-700">National Admin</span>
                </label>
