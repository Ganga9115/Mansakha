...
// State Admin's own copy of the district drill-down view - reached via
// /stateadmin/district/:jurisdictionId from StateDashboard.jsx's "Drill Down"
// button. Same case-level detail shape District Admin's own AdminDashboard
// shows for its own district, just reached as a specific child rather than
// "my own jurisdiction" (always resolved from the URL param here, never
// from the caller's own jurisdiction).
                  ...
                  {loading ? (
                    ...
                    <tr key={item.userId} className="hover:bg-gray-50/70 transition">
                      ...
                      <td className="py-3.5 px-6 text-right">
                        <button
                          onClick={() => navigate(`/stateadmin/case-detail/${item.userId}`)}
                  ...
                  {trend.map((t) => (
                    <div key={t.period} className="flex items-center gap-3 text-xs">
                      ...
                      <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-brand-700 rounded-full" style={{ width: `${Math.min(100, t.avgScore)}%` }} />
                      </div>
