...
export default function Analysis() {
  ...
  // Cross-District Comparison's own companion to the flat agencyReferralVolume
  // above - one row per district, same time window, feeding the BarChart in
  // the Cross-District Comparison section below (not the Coordination &
  // Case Handling one, which stays a flat jurisdiction-wide aggregate).
  const agencyReferralVolumeByDistrict = analyticsData?.agencyReferralVolumeByDistrict || [];
        ...
        <section className="space-y-4 pt-4 border-t border-gray-200/80">
          ...
          <ChartStatus pending={chartsPending} invalid={customRangeInvalid} loading={analyticsLoading} error={analyticsError}>
            <div className="space-y-6">
              ...
              {/* State's own Cross-District comparison for the same metric -
                  which district is generating the most coordination-role
                  workload, not just which role. Reuses the same BarChart
                  component the Cross-District Comparison section above uses. */}
              <BarChart
                title="Cross-Agency Referral Volume by District"
                subtitle="Highest first - which district is generating the most coordination-role workload this period."
                items={agencyReferralVolumeByDistrict.map((r) => ({ jurisdictionId: r.jurisdictionId, name: r.name, value: r.totalCount }))}
