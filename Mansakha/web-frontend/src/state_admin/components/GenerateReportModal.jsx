...
// State Admin's own "Generate Report" modal - a district-wise report (each
// child district's rollup, side by side, see ReportSnapshotView's
// `districts` table) submitted upward to State's own National (required,
// added server-side - never sent by this form) plus an optional direct cc
// to Ministry (the "Share With" checklist below - only checked ON goes into
// the request's `recipients` array).
...
export default function GenerateReportModal({ jurisdictionId, onClose, onGenerated }) {
  ...
  // Best-effort label for the locked/informational "National Admin" row - a
  // correct generic label beats a wrong specific one, so this falls back to
  // generic text below whenever the lookup hasn't resolved yet. Every
  // state's parent is the SAME single national jurisdiction (mirrors
  // ministry/components/ReportBuilder.jsx's own derivation), so this is just
  // the sole level='national' row - no id is sent for it since it's added
  // server-side automatically.
  const nationalOptions = useJurisdictionOptions('national');
  const nationalName = nationalOptions.data?.jurisdictions?.[0]?.name;
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h3 className="font-bold text-sm text-gray-800">Generate District-Wise Report</h3>
