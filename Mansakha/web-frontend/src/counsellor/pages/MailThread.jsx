  ...
  return (
    ...
    >
      <div className="flex flex-col h-full bg-white rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
        <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-brand-50/60 shrink-0">
          <button
          ...
          {loading && !data ? (
            ...
            return (
              <div key={m.messageId} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                <div className={`max-w-[90%] sm:max-w-[80%] md:max-w-[70%] px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-2xl text-xs sm:text-sm leading-relaxed whitespace-pre-wrap ${isMine ? 'bg-brand-700 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-800 rounded-tl-sm'}`}>
