...
// DLSA Coordinator's Legal Aid Queue - a clean, scannable list only. Every
// action (notes, lawyer assignment, hand-off, task assignment) now lives on
// the dedicated Referral Detail page (see ReferralDetail.jsx), matching the
// list -> detail convention set by dwo/pages/ReferralQueue.jsx, rather than
// cramming every action into an inline accordion within the list itself.
...
export default function LegalAidQueue() {
  ...
  return (
    <StaffLayout title="Legal Aid Queue">
      <div className="space-y-4">
        <div className="bg-white p-4 rounded-xl border border-gray-200/80 shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-sm text-gray-800">Legal Aid Referrals</h3>
            <p className="text-[11px] text-gray-400">Cases a District Admin has referred to you for legal aid follow-up.</p>
          </div>
                    ...
                    <tr key={r.referralId} className="hover:bg-gray-50/70 transition">
                      ...
                      <td className="px-6 py-3.5">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {r.metadata?.assignedLawyer && (
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-100 text-sky-700">Lawyer assigned</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-3.5 text-right">
                        <button
                          onClick={() => navigate(`/dlsa/referrals/${r.referralId}`)}
