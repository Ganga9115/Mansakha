...
// this representative's own Active cases, soonest first. A hearing ROW here
// is itself the most recent recorded outcome's own next_hearing_date; the
// actual outcome for a given hearing is recorded from the case's own detail
// page (My Cases -> View -> Record Hearing Outcome), not from here.
    ...
    <StaffLayout title="Hearings">
      <div className="bg-white p-4 rounded-xl border border-gray-200/80 shadow-sm space-y-4">
        <div>
          ...
          <p className="text-[11px] text-gray-400">Across every Legal Aid case currently assigned to you - for hearing preparation.</p>
        </div>
        ...
        <div className="border border-gray-200/80 rounded-xl overflow-hidden overflow-x-auto">
          {query.loading ? (
            ...
            <p className="text-sm text-gray-400 p-6">No upcoming hearings scheduled.</p>
          ) : (
            <table className="w-full text-left min-w-[560px]">
              ...
              <tbody className="divide-y divide-gray-100">
                {hearings.map((h) => (
                  <tr key={h.hearingId} className="hover:bg-gray-50/70 transition">
                    ...
                    <td className="px-6 py-3.5 text-xs text-gray-500">{h.court || '—'}</td>
                    <td className="px-6 py-3.5 text-xs text-gray-500">{h.hearingType || '—'}</td>
