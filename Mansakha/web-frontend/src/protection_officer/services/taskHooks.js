...
// Structured task hooks - Protection Officer's copy (mirrors
...
export function useTasksList(status) {
  ...
  return useQuery(() => {
    ...
    return apiClient.get(`/api/protectionofficer/tasks?${params.toString()}`, token);
  }, [token, status]);
}
...
export function useCreateTask() {
  ...
  const mutate = async (userId, assignedToRole, action, dueAt, sourceReferralId) => {
    setLoading(true);
    try {
      return await apiClient.post('/api/protectionofficer/tasks', { userId, assignedToRole, action, dueAt, sourceReferralId }, token);
    } finally {
      setLoading(false);
    }
  };
