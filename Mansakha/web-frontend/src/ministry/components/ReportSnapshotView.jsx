...
// Ministry's own copy of the structured report-snapshot renderer, used by
// ReportsInbox.jsx's ReportRow - replaces the raw
// <pre>{JSON.stringify(r.snapshot, null, 2)}</pre> dump with real markup,
// switching on snapshot.tier. Ministry only ever RECEIVES National's
// tier='national' (state-wise) snapshots in practice, but this is built
// generically off snapshot.tier anyway so it doesn't silently break if that
// ever changes - and so it stays a byte-similar copy of the same component
// every admin tier's own Reports.jsx now uses, per the no-shared-imports
// convention.
...
const RISK_BADGE = {
  ...
  Low: 'text-brand-800 bg-brand-50',
};
      ...
      <div className="flex items-end gap-1.5 h-16">
        {trend.map((t, i) => (
          <div key={i} className="flex-1 h-full flex flex-col items-center justify-end">
            <div
              className="w-full bg-brand-900 rounded-sm"
