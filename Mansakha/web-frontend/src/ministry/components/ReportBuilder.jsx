...
// has a jurisdiction in hand skip the picker - ReportsInbox.jsx (Ministry,
// jurisdiction-unrestricted) has no such fixed context, so it renders the
// full picker below.
export default function ReportBuilder({ onClose, onSubmitted, defaultJurisdictionId, defaultLevel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Ministry generates reports for ANY jurisdiction level (district/
      // state/national), unlike a tier-scoped admin - so this reuses
      // whichever tier the picked jurisdiction actually belongs to, since
      // each tier's own admin route only accepts jurisdictionIds inside its
      // own subtree (Ministry bypasses that check entirely, but the route
      // still needs to exist under a real mount).
      const mountByLevel = { district: 'district', state: 'state', national: 'national' };
      await apiClient.post(`/api/admin/${mountByLevel[level]}/reports/generate`, {
        jurisdictionId,
