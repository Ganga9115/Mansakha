...
// Ministry's own copy of the shared Settings/Profile screen - uses its own
// MinistryLayout chrome directly (a Layout prop is no longer needed now
// that this file only ever serves Ministry).
export default function Settings() {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      // Ministry accounts live in the same `officials` table as every other
      // staff role, so the shared change-password endpoint (keyed off
      // req.auth.officialId from the token, not role name) works here too.
    ...
    <MinistryLayout title="Profile">
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <button
                ...
                className="text-xs text-brand-800 font-semibold mt-1 hover:underline disabled:opacity-60"
              >
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-200/80 shadow-sm space-y-3">
          <h3 className="font-bold text-sm text-gray-800">Ministry Details</h3>
            ...
            {showPasswordForm && (
              <div className="p-4 bg-brand-50 rounded-lg space-y-3">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-brand-900 text-white rounded-lg text-xs font-semibold disabled:opacity-60"
                >
