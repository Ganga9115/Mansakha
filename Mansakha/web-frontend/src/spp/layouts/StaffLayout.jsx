...
import { Landmark, ClipboardList, User, LogOut, Menu, X } from 'lucide-react';
import { logout } from '../services/auth';
import { useMe } from '../services/hooks';

// Special Public Prosecutor's own dedicated shell - copied from
// dwo/layouts/StaffLayout.jsx (itself copied from district_admin's
// template).
const NAV_ITEMS = [
  { name: 'Trial Docket', icon: Landmark, path: '/spp' },
  { name: 'My Tasks', icon: ClipboardList, path: '/spp/tasks' },
  { name: 'Profile', icon: User, path: '/spp/profile' },
];

export default function StaffLayout({ children, title = 'Trial Docket' }) {
        ...
        <header className="h-16 bg-[#EBF4FA] border-b border-[#D6E8F5] px-4 sm:px-8 flex items-center justify-between shrink-0 gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-5 shrink-0">
            <div className="text-xs hidden sm:block text-right">
              ...
              <p className="text-[#3D5A80]/70">Special Public Prosecutor</p>
            </div>
