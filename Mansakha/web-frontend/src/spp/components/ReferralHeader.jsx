...
// Shared summary bar reused across Overview/Activity Log/Tasks. Mirrors
// dwo/components/ReferralHeader.jsx (the template), with Stage shown
// alongside Status - the real docket stage (Docket Received -> Hearing
// Scheduled -> Verdict Delivered), not just the coarse Open/Resolved gate.
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-5 rounded-xl border border-gray-200/80 shadow-sm flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-10 flex-wrap">
          ...
          {r.stage && (
            <div>
              <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase block mb-1">Stage</span>
              <span className={`text-xs font-bold px-2.5 py-0.5 rounded ${STAGE_BADGE[r.stage] || 'bg-gray-100 text-gray-600'}`}>{r.stage}</span>
            </div>
          )}
          <div>
