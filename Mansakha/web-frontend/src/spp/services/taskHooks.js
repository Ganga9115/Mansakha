...
// Structured task hooks - Special Public Prosecutor's copy (mirrors
...
export function useTasksList(status) {
  ...
  return useQuery(() => {
    ...
    return apiClient.get(`/api/spp/tasks?${params.toString()}`, token);
  }, [token, status]);
}
...
export function useCreateTask() {
  ...
  const mutate = async (userId, assignedToRole, action, dueAt, sourceReferralId) => {
    setLoading(true);
    try {
      return await apiClient.post('/api/spp/tasks', { userId, assignedToRole, action, dueAt, sourceReferralId }, token);
    } finally {
      setLoading(false);
    }
  };
...
export function useCompleteTask() {
  ...
  const mutate = async (taskId) => {
    setLoading(true);
    try {
      return await apiClient.patch(`/api/spp/tasks/${taskId}/complete`, {}, token);
    } finally {
      setLoading(false);
    }
  };
