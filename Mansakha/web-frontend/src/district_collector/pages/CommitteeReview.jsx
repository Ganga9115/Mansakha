...
// District Collector's Committee Review - a clean, scannable list only.
// Every action (cross-agency view, directives, task assignment) now lives
// on the dedicated Review Detail page (see ReviewDetail.jsx), matching the
// list -> detail convention already used by district_admin's own Case File
// view, rather than cramming every action into an inline accordion.
...
export default function CommitteeReview() {
  ...
  return (
    <StaffLayout title="Committee Review">
      <div className="space-y-4">
        <div className="bg-white p-4 rounded-xl border border-gray-200/80 shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-sm text-gray-800">Vigilance &amp; Monitoring Committee Review</h3>
            <p className="text-[11px] text-gray-400">Cases a District Admin escalated, or the system flagged automatically. Select a case to see every agency's work on it before logging a directive. Read-only elsewhere; District Admin keeps full decision authority.</p>
          </div>
          ...
          <div className="border border-gray-200/80 rounded-xl overflow-hidden overflow-x-auto">
            {query.loading ? (
              ...
              <p className="text-sm text-gray-400 p-6">No {tab.toLowerCase()} reviews.</p>
            ) : (
              <table className="w-full text-left min-w-[640px]">
                <thead>
                  <tr className="bg-gray-50 text-[11px] font-bold text-gray-500 uppercase">
                    ...
                    <th className="px-6 py-3">Escalated On</th>
