...
// Structured task hooks - District Collector's copy (mirrors
...
export function useTasksList(status) {
  ...
  return useQuery(() => {
    ...
    return apiClient.get(`/api/districtcollector/tasks?${params.toString()}`, token);
  }, [token, status]);
}
...
export function useCompleteTask() {
  ...
  const mutate = async (taskId) => {
    setLoading(true);
    try {
      return await apiClient.patch(`/api/districtcollector/tasks/${taskId}/complete`, {}, token);
    } finally {
      setLoading(false);
    }
  };
