...
// Investigating Officer's Case Status Feed - a clean, scannable list only.
// Every action (notes, resolve, task assignment) now lives on the
// dedicated Referral Detail page (see ReferralDetail.jsx), matching the
// list -> detail convention established by dwo/pages/ReferralQueue.jsx,
...
// distress) computed server-side from Accused Status + sos_events history.
...
export default function CaseStatusFeed() {
  ...
  return (
    <StaffLayout title="Case Status Feed">
      <div className="space-y-4">
        <div className="bg-white p-4 rounded-xl border border-gray-200/80 shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-sm text-gray-800">Threat Assessment Referrals</h3>
            <p className="text-[11px] text-gray-400">Cases a District Admin has referred to you for intimidation/threat assessment.</p>
          </div>
                  ...
                  {referrals.map((r) => (
                    <tr key={r.referralId} className="hover:bg-gray-50/70 transition">
                      ...
                      <td className="px-6 py-3.5 text-right">
                        <button
                          onClick={() => navigate(`/io/referrals/${r.referralId}`)}
