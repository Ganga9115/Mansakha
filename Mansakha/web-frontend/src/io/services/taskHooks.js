...
// Structured task hooks - Investigating Officer's copy (mirrors
...
export function useTasksList(status) {
  ...
  return useQuery(() => {
    ...
    return apiClient.get(`/api/io/tasks?${params.toString()}`, token);
  }, [token, status]);
}
...
export function useCreateTask() {
  ...
  const mutate = async (userId, assignedToRole, action, dueAt, sourceReferralId) => {
    setLoading(true);
    try {
      return await apiClient.post('/api/io/tasks', { userId, assignedToRole, action, dueAt, sourceReferralId }, token);
    } finally {
      setLoading(false);
    }
  };
