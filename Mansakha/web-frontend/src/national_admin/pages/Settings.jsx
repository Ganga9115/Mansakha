...
// National Admin's own copy of the shared Settings/Profile screen -
  ...
  return (
    <StaffLayout title="Profile">
      <div className="space-y-6 max-w-5xl">
        ...
        {/* Hero Card */}
        <div className="relative bg-gradient-to-r from-[#f0f7fb] to-[#e4f1f9] rounded-2xl border border-blue-100 shadow-sm overflow-hidden p-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
          {/* Watermark Icon */}
          <Landmark size={140} className="absolute right-1/3 top-1/2 -translate-y-1/2 text-blue-200/40 pointer-events-none" />
          ...
          <div className="flex items-center gap-6 relative z-10">
            <div className="relative">
              {me?.profileImageUrl ? (
                ...
                <div className="w-24 h-24 rounded-full bg-white text-[#2a77c8] font-bold text-3xl flex items-center justify-center border-4 border-white shadow-sm shrink-0">
                  {me?.fullName ? me.fullName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'NA'}
                </div>
              )}
              ...
              <button 
                ...
                className="absolute bottom-0 right-0 w-8 h-8 bg-[#2a77c8] hover:bg-[#2363a8] text-white rounded-full flex items-center justify-center border-2 border-white shadow-sm transition-colors"
            ...
            <div className="space-y-1">
              <h2 className="text-xl font-extrabold text-[#1a2b4b]">{me?.fullName || 'Loading...'}</h2>
              <p className="text-[13px] font-medium text-gray-600">{jobTitle}</p>
              <div className="flex items-center gap-1.5 text-gray-500 mt-1">
                <Mail size={14} />
                <span className="text-[13px]">{me?.email || 'national.admin@mansakha.gov.in'}</span>
              </div>
              <button
                onClick={() => fileInputRef.current?.click()}
                disabled={photoUploading}
                className="text-[12px] font-bold text-[#2a77c8] hover:underline mt-2 inline-block disabled:opacity-60"
              >
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-blue-50/50 w-full md:w-64 shrink-0 relative z-10">
            <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider block mb-2">Jurisdiction</span>
            <div className="flex items-center gap-2 mb-3">
              <MapPin size={16} className="text-[#2a77c8]" />
              <span className="text-[14px] font-extrabold text-[#1a2b4b]">
                {role?.jurisdictionName ? `${role.jurisdictionName} (National)` : 'National'}
              </span>
            </div>
            <p className="text-[11px] text-gray-500 leading-relaxed">
              Overseeing cases and coordination across the nation.
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-50">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-blue-50 text-[#2a77c8] rounded-xl flex items-center justify-center shrink-0">
                <BadgeCheck size={24} />
              </div>
              <div>
                <h3 className="text-[16px] font-extrabold text-[#1a2b4b]">Administration Details</h3>
                <p className="text-[13px] text-gray-500 mt-0.5">Your role and access information.</p>
              </div>
            </div>
            <button className="px-4 py-2 border border-gray-200 text-[#2a77c8] rounded-lg text-xs font-bold hover:bg-blue-50 transition-colors flex items-center gap-2 self-start md:self-auto">
              <Pencil size={12} /> Edit Details
            </button>
          </div>
          
          <div className="p-6 space-y-0">
            <DetailRow label="Jurisdiction" value={role?.jurisdictionName ? `${role.jurisdictionName} (National)` : 'National'} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[16px] font-extrabold text-[#1a2b4b]">Account Security</h3>
          ...
          <div className="px-6 pb-6">
            <div className="bg-[#f8fafc] rounded-xl p-5 border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-full bg-white border border-gray-200 text-[#2a77c8] flex items-center justify-center shrink-0 mt-0.5">
                  <Key size={14} />
                </div>
                <div>
              ...
              <div className="flex flex-col gap-2 min-w-[200px]">
                <button
                  ...
                  className="px-5 py-2.5 bg-[#2a77c8] hover:bg-[#2363a8] text-white rounded-lg text-[13px] font-bold transition-colors flex items-center justify-center gap-2 w-full md:w-auto"
                >
              ...
              <div className="mt-4 p-5 bg-white border border-gray-200 rounded-xl space-y-4 max-w-md ml-auto">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 pr-10 border border-gray-300 rounded-lg text-xs focus:outline-none focus:border-[#2a77c8] focus:ring-1 focus:ring-[#2a77c8]"
                  />
                  <button
                    type="button"
