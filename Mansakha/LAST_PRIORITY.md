...
- **Institutional-portal redesign** (flat borders, DataTable, StatCard,
  AlertBanner, StatusBadge, GovernmentHeader, Public Sans, no animation)
  landed on the shared components + every Staff/Ministry screen. A few
  scoped simplifications worth naming: the header's alert-count badge
  renders but isn't fed live data yet (would need a per-role alert-count
  fetch wired into `StaffShell.js`); the jurisdiction context zone shows
  the tier ("District tier") rather than the resolved jurisdiction *name*
  (e.g. "Pune") since no cheap query returns that at the shell level today;
  `DataTable`'s `accessibilityRole="table"/"row"/"columnheader"/"cell"`
  values aren't part of React Native's own official role enum - they pass
  through to real ARIA roles on web via react-native-web, but haven't been
  verified against a screen reader, only reasoned about.
- **Victim's own screens (Home, Check-in, History, Support, Settings)
  intentionally kept their existing `Card`/`StatGrid` treatment** - this
  pass was scoped to Staff/Ministry per the redesign spec; they already
  share the same underlying `Button`/`Card`/`IconInput`/`Toast`/`Skeleton`
  component changes (flat, no animation) since those are shared, just not
  converted to `DataTable`.
- **Victim signup's district picker is now a real State -> District
  cascade** (added this pass) - narrows the district list to the chosen
  state instead of one flat "District, State" list, making the existing
  jurisdiction-scoping (already enforced server-side via
  `requireJurisdiction`) legible at signup time too.

- **New page-layout system (`theme/layout.js`, `ScreenContainer`, `Section`,
  `StatGrid`) covers the highest-traffic screens** - Victim Home, Counsellor
  Dashboard, Administration Dashboard (with the Export CSV button also
  wired up now - it had a working backend endpoint but no frontend button
  until this pass, a real gap this closes), Support, Settings, Staff
  Management, System Config, Case Queue, Alerts Feed (both), Audit Log,
  Workload, Distress History. Not yet applied: `CaseDetailScreen.js`,
  `LogInterventionScreen.js`, `ChangePasswordScreen.js`, `ConsentScreen.js`,
  `CheckinScreen.js`/`CheckinConfirmationScreen.js` - same mechanical
  `ScreenContainer` wrap would apply cleanly, just not reached yet.


- **The reanimated-driven interactivity layer only reached auth so far.**
  `theme/motion.js` + the new split-panel `AuthLayout.js`/`Stepper.js`/
  `AuthModeSelect.js` are real and live (note: this also fixed a real
  latent setup bug - `react-native-reanimated` had no `babel.config.js`
  registering its plugin at all until this pass, so its worklets were
  never actually functional before). The planned second half - staggered
  list-item enter animations on Case Queue/Alerts Feed/Audit Log/Staff
  list, and spring-based press feedback on `Button`/`Card`/`IconInput` -
  wasn't reached this session; those screens still use the Phase 2
  static-style-function press feedback, which still works, just isn't
  animated.
